...
const RestroCard = (props) => {
  const item =
    props.resData && props.resData.data
      ? props.resData.data
      : props.resData || {};
  const cuisines = (item.cuisines || []).join(", ");
  const name = item.name || "Unknown";
  const ratings = item.avgratings || "N/A";
  const costfortwo = item.costForTwo ? item.costForTwo / 100 : "-";
  ...
  return (
    <div className="ResCard">
      <img src={imageURL} alt="App Logo" className="RestImage" />
      <h3 className="Restname">{name}</h3>
      <h4 className="Cuisines">{cuisines}</h4>
      <h4 className="Ratings">{ratings}</h4>
      <h4 className="CostForTwo">{costfortwo + " Fow Two"}</h4>
      <h4 className="DeliveryTime">{deliveryTime + " min"}</h4>
    </div>
  );
};
  ...
  return (
    <div className="body">
      ...
      <div className="RestroContainer">
        {resList.map((rest) => (
          <RestroCard key={rest.data?.id || rest.data?.uuid} resData={rest} />
        ))}
      </div>
    </div>
  );
